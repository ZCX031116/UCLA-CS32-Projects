...
{}
void MovieDatabase::Clear(std::string& id, std::string& title, std::string& release_year, std::string& directors,
    std::string& actors, std::string& genres, std::string& Rating)
{
    id.clear();
    title.clear();
    release_year.clear();
...
void MovieDatabase::load_helper(std::string id, std::string title, std::string release_year, std::string directors,
std::string actors, std::string genres, std::string Rating)
{
    std::string key1;
    std::string key2;
...
bool MovieDatabase::load(const string& filename)
{
    ...
    while (std::getline(input_file, line))
    {
        if (line.empty())
        {
            load_helper(id,title,release_year,directors,actors,genres,Rating);
            Clear(id, title, release_year, directors, actors, genres, Rating);
        }
        else if (id.empty())
            id = line;
        else if (title.empty())
